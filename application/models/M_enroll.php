...
class M_enroll extends CI_Model{
	...
	function get_current_sy_sem()
    {
        ...
        $this->db->where(array('A.SyId' => 9, 'A.SemId' => 3));
        $query = $this->db->get();
        $row = array();
        if($query->num_rows() > 0)
        {
	...
	{
		$curriculum = $this->db->where('CurriculumId', $curriculum_id)->get('tblcurriculum')->row_array();
		// $curriculum = $this->session->userdata('session')['student_curriculum'];

		if ( ! empty($curriculum['SummerYearLevel']))
		{
		    $summerYrLevel = explode(",", $curriculum['SummerYearLevel']);

		    $loop  = 0;
		    $yrLvl = 0;
		    $year  = array('First', 'Second', 'Third', 'Fourth', 'Fifth');

		    $data['semester'] = array();
		    $data['semester'][] = 'Open Semester';
		    foreach ($year as $level)
		    {
		        $yrLvl++;
		        $loop = $loop + 2;
		        $data['semester'][] = $level . ' Year';
		        $data['semester'][] = $level . ' Year';

		        if (in_array($yrLvl, $summerYrLevel))
		        $data['semester'][] = $level . ' Year';

		        if ($loop >= (int)$curriculum['NoOfSem'])
		        break;
	...
	public function get_total_units($student_id, $sy_id, $sem_id)
	{
		$this->db->select_sum('C.Units', 'Units');
		$this->db->where([
							'StudNo'   => $student_id,
							'A.SyId'   => $sy_id,
							'A.SemId'  => $sem_id,
							'IsActive' => 1])
				->join('tblsched as B', 'A.Cfn = B.cfn', 'left')
				->join('tblcourse as C','B.subject_id = C.CourseId', 'left');

		$query = $this->db->get('tblstudentschedule as A')->row();

        return $query->Units ? $query->Units : 0;
	}

	public function has_reached_max_units($student_id, $sy_id, $sem_id)
	{
		$max_units 	= $this->get_max_units(
											$this->session->userdata('session')['student_curriculum']['CurriculumId'],
											$this->session->userdata('session')['student_info']['LengthOfStayBySem']
										);

		$total_units = $this->get_total_units($student_id, $sy_id, $sem_id);

		return (bool)$total_units > $max_units;
	...
	public function get_nstp_subject($studno, $sy_id, $sem_id)
	{
        ...
        $this->db->like('CourseCode', 'NSTP', 'both');
	...
	public function set_payment_schedule($enrollment_trans_id)
	{
		...
		$SyId = config_item('sy_sem')['SyId'];
		$SemId = config_item('sy_sem')['SemId'];
		...
		$sql = "INSERT INTO tblstudpaymentsched(enrollment_trans_id, payment_schedule_id)
		    		SELECT ?, payment_schedule_id FROM tblpaymentschedule as b
			    		WHERE (
			    				SELECT COUNT(*)+1 FROM tblenrollmenttrans as c
			    				WHERE c.PaymentDate = b.payment_schedule
			    					AND SyId = {$SyId} AND SemId = {$SemId}
			    			  ) <= b.max_student
		...
		if ( ! empty($payment_date['payment_schedule']))
		{
		...
		return ( ! empty($payment_date['payment_schedule'])) ? $payment_date['payment_schedule'] : '';
	...
	*/
    ...
    {
    	if ($this->session->userdata('session')['student_info']['AllowAccessTo1stYrSched'] != 1)
    	{
	    	...
	    	foreach($query->result_array() as $row)
	    	{
				...
				else if(in_array(strtoupper($remarks['Remarks']), $filter_remarks) || empty($remarks['Remarks']))
				{
				...
				else if(strtoupper($remarks['Remarks']) == 'NOT TAKEN')
					{
						...
						if ($this->session->userdata('session')['student_info']['LengthOfStayBySem'] == 1)
						{
							// var_dump($this->session->userdata('session')['student_curriculum']);
							if(($length_of_stay >= $row['LengthOfStayBySem']) || ($this->session->userdata('session')['student_curriculum']['CollegeId'] == 11 && $this->session->userdata('session')['student_curriculum']['Controlled'] == 1))
							{
	...
	function get_block_section_schedule($sy_id,$sem_id,$curriculum_id,$length_of_stay_by_sem,$student_id,$program_id,$college_id,$major_id)
    {
        ...
        else
        {
	    	...
	    	echo "<span style='display:none;'>$year_level</span>";
    ...
    {
        $this->db->select_sum('Units', 'Units');
        $query = $this->db->where('CurriculumId', $curriculum_id)
				        ->where('LengthOfStayBySem', $length_of_stay_by_sem)
				        ->join('tblcourse as B','A.CourseId = B.CourseId')
				        ->get('tblcurriculumdetails as A')
        ...
        $max_units = $query->Units ? $query->Units : 0;

        $allowed_overload_units = $this->get_allowed_overload_units(
        									$curriculum_id, 
        									$length_of_stay_by_sem
        								);

		if (count($allowed_overload_units))
		{
			if ($allowed_overload_units['NotToExceed'] > 0)
    		$max_units = $allowed_overload_units['NotToExceed'];
        	else
    		$max_units += $allowed_overload_units['AdditionalUnits'];
		}
    ...
    function update_enrollment_trans($enrollment_trans,$edte,$adts)
    {
        ...
        if ($edte)
        {
	        ...
	        $_SESSION['session']['enrollment_trans']['IsEncoded'] = 1;
	        $_SESSION['session']['enrollment_trans']['EncodedDateTimeEnd'] = $date_now;
        }

        if ($adts)
        {
            $data = array('AssessedDateTime'=>$date_now, 'IsAssessed'=>'1');
	        $_SESSION['session']['enrollment_trans']['IsAssessed'] = 1;
	        $_SESSION['session']['enrollment_trans']['AssessedDateTime'] = $date_now;
        }
    ...
    {
        $studno = $this->session->userdata('session')['student_info']['StudNo'];
        $SyId = config_item('sy_sem')['SyId'];
        $SemId = config_item('sy_sem')['SemId'];
    ...
    function set_user_logs($logs)
    {
        ...
        $data = array('student_id' => $this->session->userdata('session')['student_info']['StudNo'], 'action' => $logs,'date_logged' => $date_t);
    ...
    function add_student_losby($student_id, $sy_id, $sem_id)
    {
		...
		if ($sem_id == 1)
		{
			if (in_array($student_curriculum->CurriculumId, [184, 181, 179, 182,144]))
			{
				...
				{
					// if (in_array($student_curriculum->CurriculumId, [182] ))
					// {
						if (in_array($student->LengthOfStayBySem, [2,5,8]))
						{
							$this->db->where(array('SemId' => 3,
													'SyId' => $summer_sy_id,
													'StudNo' => $student_id
											));
							$summer = $this->db->get('tblenrollmenttrans');
							if ($summer->num_rows() == 0)
							$promotion++;
						}
					// }
    ...
    {
    	$id = $this->session->userdata('session')['enrollment_trans']['Id'];
    	$data = array(
    	...
    	else
    	{
	    	if ( ! empty($student_id))
	    	{
		    	...
		    	$this->db->limit(1)->update('tblenrollmenttrans', $data);
		    	return $this->db->affected_rows();
	    	}
    	}
    ...
    public function is_nstp2_subject($cfn, $sy_id, $sem_id)
    {
    	$this->db->where(array(
    							'cfn' => $cfn,
    							'SyId' => $sy_id,
    	...
    	if (!empty($sched))
    	{
    		if (in_array($sched->subject_id, $this->nstp2_subjects))
    		{
    			return $sched->subject_id;
    		}
    	}
    ...
    public function is_nstp_preq($student_id, $subject_id)
    {
    	$nstp1_subject_id = $this->nstp2_pre_req[$subject_id];
    	$this->db->where(array(
    						'StudNo' => $student_id,
    						'CourseId' => $nstp1_subject_id,
    						'is_actived' => 1
				    	));
    	$this->db->where('(UPPER(Remarks) IN("PASSED", "INCOMPLETE", "INC") OR IsCompleted = 1)', NULL, FALSE);
    	$this->db->select('StrGrade, Remarks');
    	$nstp_grade = $this->db->get('tblstudgrade')->row();
    ...
    public function get_enrollment_sched($param)
    {
    	$this->db->where('enroll_sched_is_active', 1);
    	return $this->db->where($param)->get('tblenrollmentschedule')->row_array();
    }

    public function nstp_subject_by_college($college_id)
    {
    	$this->db->where('CollegeId', $college_id);
    	return $this->db->select('CFN, subject_id')->get('tblschednstp')->result_array();
    }
